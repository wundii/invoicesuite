...
function getMarkDown($prevTag, $currTag)
{
    ...
    if (is_null($commitStr) || false === $commitStr) {
        return $markDown;
    }
    ...
    $commits = array_filter($commits, static function ($commit) use (&$noOfHiddenCommits) {
    ...
    if ([] !== $commits) {
        ...
        foreach ($commits as $commit) {
            ...
            $commitIssuesWithUrls = array_map(static fn ($issue) => sprintf('[%1$s](https://github.com/horstoeko/invoicesuite/issues/%2$s)', $issue, substr((string) $issue, 1)), $commitIssues);
            ...
            $markDown[] = sprintf('| %6$s | [%1$s](https://github.com/horstoeko/invoicesuite/commit/%1$s) | %2$s | %3$s | %4$s | %5$s', $commitHash, $commitDate, $commitAuthor, $commitSubject, implode(', ', $commitIssuesWithUrls), $commitSubjectIcons);
        }

        $markDown[] = '';
    }

    if (1 === $noOfHiddenCommits) {
